...
class ExplorerTab:
    ...
    def _build(self) -> None:
        ...
        # ── header strip inside the output frame ──────────────────────
        self._ai_out_hdr = tk.Frame(self._ai_out_frame, bg=C["accent"], height=32)
        self._ai_out_hdr.pack(fill=tk.X)
        self._ai_out_hdr.pack_propagate(False)
        self._ai_out_title = tk.Label(
            self._ai_out_hdr,
            text="AI Time-Range Summary",
            bg=C["accent"], fg=C["white"], font=F_BOLD, padx=12,
            anchor="w",
        )
        self._ai_out_title.pack(side=tk.LEFT, fill=tk.Y)

        # ── meta strip (range + event count) ──────────────────────────
        self._ai_out_meta = tk.Frame(self._ai_out_frame, bg=C["surface3"], pady=3)
        self._ai_out_meta.pack(fill=tk.X)
        self._ai_out_meta_lbl = tk.Label(
            self._ai_out_meta, text="",
            bg=C["surface3"], fg=C["text_s"], font=F_SMALL, padx=12,
            anchor="w",
        )
        self._ai_out_meta_lbl.pack(fill=tk.X)
        ...
        self._ai_out_txt = scrolledtext.ScrolledText(
            ...
            height=7, state=tk.DISABLED, padx=14, pady=10)
        self._ai_out_txt.pack(fill=tk.X)

        # Text tags for formatting
        self._ai_out_txt.tag_configure(
            "sentence_bold", font=F_BOLD, foreground=C["text"])
        self._ai_out_txt.tag_configure(
            "accent_line", foreground=C["accent"], font=F_BOLD)
        self._ai_out_txt.tag_configure(
            "muted", foreground=C["text_s"], font=F_SMALL)
    ...
    def _show_range_summary(
        ...
        # ── Update header and meta strip ──────────────────────────────
        self._ai_out_title.configure(
            text=f"AI Time-Range Summary  ·  {from_str}  →  {to_str}")
        self._ai_out_meta_lbl.configure(
            text=f"  Events in range: {total}   |   Analysed by AI: {sent}"
                 f"   |   Channel: {self._channel_var.get()}")

        # ── Render body with light formatting ────────────────────────
        txt = self._ai_out_txt
        txt.configure(state=tk.NORMAL)
        txt.delete("1.0", tk.END)

        # Clean up the raw AI response:
        # • Remove any leading/trailing whitespace
        # • Split into sentences and render each on its own line with
        #   the first sentence of each paragraph bolded
        cleaned = summary.strip()
        if not cleaned:
            txt.insert(tk.END, "No response received from AI.", "muted")
        else:
            # Split into paragraphs first (double newline), then sentences
            ...
            for pi, paragraph in enumerate(paragraphs):
                ...
                import re as _re
                sentences = _re_split_sentences(paragraph)
                for si, sentence in enumerate(sentences):
                    sentence = sentence.strip()
                    if not sentence:
                        continue
                    if si == 0 and pi == 0:
                        # First sentence of the whole response — bold accent
                        txt.insert(tk.END, sentence + "\n", "accent_line")
                    elif si == 0:
                        # First sentence of subsequent paragraphs — bold
                        txt.insert(tk.END, "\n" + sentence + "\n", "sentence_bold")
                    else:
                        txt.insert(tk.END, sentence + " ", "")
